...
OptimizationResult PoseOptimizer::optimize_pose(std::shared_ptr<Frame> frame) {
    ...
    // TODO: Get camera parameters from Config or pass as parameter
    // For now using placeholder values - this should be configurable
    ...
    for (size_t i = 0; i < frame->m_map_points.size(); ++i) {
        auto mp = frame->m_map_points[i];
        if (!mp || mp->is_bad()) {
            continue;
        }
    ...
    if (m_config.enable_outlier_detection) {
        for (int round = 0; round < m_config.outlier_detection_rounds; ++round) {
            ...
            detect_outliers(const_cast<double const* const*>(&pose_params.data()), 
